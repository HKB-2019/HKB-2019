...
npm run admin:password   # prints ADMIN_PASSWORD_HASH and SESSION_SECRET for .env
npm run seed             # load the catalogue and stock
npm run server           # API on :3001
npm run dev              # front end on :5173, proxies /api to the API
npm test                 # the money tests and the admin tests
```

Three pages: `/` the shop, `/order` where Paystack returns the customer, and
`/admin` the back room. In development Vite serves all three; in production
`npm run build` then `npm run server` serves `dist/` from the API process, with
any non-`/api` path falling back to `index.html` so a refresh on `/admin` works.
...
scrypt hash of it, in `ADMIN_PASSWORD_HASH`, generated by `npm run admin:password`.

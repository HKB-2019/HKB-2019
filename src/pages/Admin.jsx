...
import {
  adminLogin, adminLogout, adminMe, adminOrders,
  adminSummary, adminStock, adminSetStock, adminFulfil
} from '../lib/api.js';

const naira = (kobo) => '₦' + (kobo / 100).toLocaleString('en-NG');
const when = (iso) => iso ? new Date(iso.replace(' ', 'T') + 'Z').toLocaleString('en-GB', {
...
export default function Admin() {
  ...
  useEffect(() => {
    adminMe().then(() => setSignedIn(true)).catch(() => setSignedIn(false));
  }, []);
